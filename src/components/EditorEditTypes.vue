...
const save = async(item: Partial<EditType>) => {
  if (editId.value) {
    await editTypes.update({ name: item.name! }, editId.value)
    editId.value = null
  }
  else { await editTypes.add(item) }
...
<template>
  <section class="card">
      ...
      <Dialog v-model="dialogOpen" class="max-w-sm">
        <header class="p-4">
          <span class="text-gray-600">
            {{ editId != null ? 'Rename Edit type' : 'Create new Edit type' }}
          </span>
        </header>
        <div class="px-4">
          <div class="flex-1">
            <label class="block mb-2 text-sm font-medium text-gray-900 dark:text-gray-300">
              <div class="pb-1">Edit type</div>
              <input
    ...
    <ul class="py-4">
      <li v-for="{ id, active, name } in editTypes.all" :key="id" class="px-4 py-2 flex justify-between">
        <div>
          {{ name }}
        ...
        <div class="flex items-center space-x-2 text-gray-600">
          <button class="p-1" @click="editTypes.update({ active: !active }, id)">
          ...
          <button class="p-1" @click="editTypes.delete(id)">
            <i-ph-trash />
          </button>
        </div>
      </li>
